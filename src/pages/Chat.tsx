    ...
    <div className="flex flex-col gap-4 p-5 h-full overflow-hidden select-none">
      ...
      <div className="glass-card rounded-2xl p-4 py-4 border border-primary/15 shadow-romantic space-y-4 animate-fade-in-up flex-shrink-0">
        <div>
          ...
          <p className="text-[13px] text-foreground italic leading-relaxed mb-2.5 bg-primary/5 p-3 rounded-xl border border-primary/10">
            "{prompt}"
          </p>
          <Button
            variant="ghost"
            ...
            onClick={onNewPrompt}
          >
            <Heart className="w-3.5 h-3.5 mr-1.5 fill-primary/20" />
            New Spark
          </Button>
        </div>

        <div className="pt-3.5 border-t border-primary/15">
          ...
          <textarea
            ...
            className="w-full bg-card/50 border border-primary/10 rounded-xl p-3 text-sm text-foreground placeholder:text-muted-foreground/50 resize-none outline-none leading-relaxed min-h-[80px] max-h-[95px] focus:ring-1 focus:ring-primary focus:border-primary transition-all shadow-inner"
          />
        </div>
      </div>
      ...
      <Button
        variant="outline"
        className="w-full rounded-full gap-2.5 h-11 border-primary/30 bg-primary/5 hover:bg-primary/10 text-primary text-sm font-semibold shadow-sm hover:shadow-md transition-all flex-shrink-0"
        onClick={onPlayGame}
      >
      ...
      <Button
        variant="outline"
        className="w-full rounded-full gap-2.5 h-11 border-border/60 hover:bg-muted text-sm shadow-sm hover:shadow-md transition-all flex-shrink-0"
    ...
    <div className="h-full max-h-full flex flex-col bg-background overflow-hidden">
      ...
      <header className="flex-shrink-0 bg-card/60 backdrop-blur-xl border-b border-primary/10 px-5 py-3.5 flex items-center gap-4 shadow-sm z-10 animate-fade-in">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-serif font-bold text-lg text-foreground leading-tight truncate">
            {partnerName}
          </p>
          <p className="text-xs text-muted-foreground flex items-center gap-1 font-medium mt-0.5">
            {partnerPresence?.online && (
              <span className="w-2 h-2 rounded-full bg-green-400 inline-block animate-pulse-glow" />
            )}
            {presenceLabel}
          </p>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <Button
            variant="ghost"
            size="icon"
            className="rounded-full w-9 h-9 text-muted-foreground hover:text-primary hover:scale-105 active:scale-95 transition-all"
          ...
          <Button
            variant="ghost"
            size="icon"
            className={`rounded-full w-9 h-9 hidden md:flex hover:scale-105 active:scale-95 transition-all ${gamePanelOpen ? "text-primary bg-primary/10" : "text-muted-foreground"}`}
          ...
          <Button
            ...
            variant="ghost"
            size="icon"
            className="rounded-full w-9 h-9 text-muted-foreground hidden md:flex hover:scale-105 active:scale-95 transition-all"
            onClick={() => { setSidebarOpen((v) => !v); setGamePanelOpen(false); }}
          >
            {sidebarOpen ? <PanelRightClose className="w-4.5 h-4.5" /> : <PanelRightOpen className="w-4.5 h-4.5" />}
          </Button>
          <Button
            data-testid="button-signout"
            variant="ghost"
            size="icon"
            className="rounded-full w-9 h-9 text-muted-foreground hover:text-destructive hover:scale-105 active:scale-95 transition-all"
            onClick={signOut}
          >
            ...
            <div className="flex-shrink-0 bg-card/80 border-t border-border px-4 py-2 flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0 border-l-2 border-primary pl-2">
                <p className="text-xs font-medium text-primary">
                  {replyingTo.isAI ? "Panda" : replyingTo.senderId === user?.uid ? "You" : partnerName}
                </p>
